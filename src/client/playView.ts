...
function usageHtml(turn: TurnResult | null): string {
  ...
  return `<section class="card usage"><strong>API 사용량 추정</strong><p>${usage.provider} · ${usage.model}</p><p>prompt ${usage.promptTokens} / completion ${usage.completionTokens} tokens</p><p class="muted">실제 비용은 provider 정책에 따라 다를 수 있습니다.</p></section>`;
}
...
export function playHtml(state: ClientUiState): string {
  ...
  return `<section class="card status-grid"><div class="pill">플레이어 HP ${snapshot?.player.hp ?? '40/40'} · MP ${snapshot?.player.mp ?? '25/25'}</div><div class="pill">적 HP ${snapshot?.enemy?.hp ?? '20/20'} · ${snapshot?.enemy?.condition ?? 'normal'}</div><div class="pill">${apiMode}</div></section>

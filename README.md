...
- Current data follows the uploaded `ManRPG_v18_통합시트기준_수정본_패키지.zip` direction; conflicts prefer the integrated sheet 기준.
...
src/gm/      template narration, local parser, compact prompt contracts
src/ai/      key security, call policy, compact payloads, response sanitization
...
- Default **이번 접속에서만 사용** keeps API keys in memory only.
- Optional **암호화 저장** derives an AES-GCM key from a user PIN/passphrase via PBKDF2; plaintext device storage is disabled and the PIN/passphrase is never stored.
...
4. Choose `sessionOnly` or encrypted device storage. Plaintext device storage is disabled.
...
The Worker is not the default LLM route. `/llm` returns a disabled/fallback response without a key, `/health` does not reveal operator key presence, and `/proxy/llm` only works when explicitly enabled. In proxy mode, UI/docs must warn that the player API key can pass through the Worker.

## LLM Output Authority

LLM responses are sanitized into compact task shapes (`n/c`, `intent/style/hint`, `intent/target/id/aim`, or skill flavor fields). Any `stateDeltas` or numeric state/effect fields are removed or rejected before game logic sees them.
